...
public class Burst7UpDown : MonoBehaviour
{
    public bool _3TargetGame = true;
    ...
    public List<Burst_Destination_Object> destinationRef_Tranform_List;
    ...
    void Start()
    {
        ...
        if (destinationRef_Tranform_List != null && destinationRef_Tranform_List.Count != 0)
        {
            ...
            foreach (Burst_Destination_Object t in destinationRef_Tranform_List)
            {
                destinationList.Add(t.t_obj.localPosition);
            }
        }
    }
    ...
    {
        Debug.Log("Anim : " + roundDuration + " " + remTime);
        _burstChk = true;
    ...
    IEnumerator MoveObject(Vector3 initialPos, Vector3 targetPos, Transform coinTransform, int destination, bool exact = false, bool randomParent = true, int c_num = -1)
    {
        ...
        if (!exact)
        {
            targetPosModified = targetPos + new Vector3(Random.Range(-destinationRef_Tranform_List[destination].areaX, destinationRef_Tranform_List[destination].areaX), Random.Range(-destinationRef_Tranform_List[destination].areaY, destinationRef_Tranform_List[destination].areaY), 0);
        }
        if (randomParent)
        {
...
[System.Serializable]
public class Burst_Destination_Object
{
